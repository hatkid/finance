 ...
 *
 */
public class ServiceLogisticsTask extends QuartzJobBean {

	private final Logger LOGGER = LoggerFactory.getLogger(ServiceLogisticsTask.class);

	@Override
	protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
		LOGGER.info("ServiceLogisticsTask is begin ...");
		ServiceLogisticsService serviceLogisticsService = SpringUtil.getBean("ServiceLogisticsService");
		long result = serviceLogisticsService.getCountNOLocation();
		recursion(result, serviceLogisticsService);
	}
	
	public void recursion(long l, ServiceLogisticsService serviceLogisticsService){
		if(l > 0){
			LOGGER.info("开始处理。。。");
			...
			LOGGER.info("ServiceLogisticsTask is over ...");
		}
	}
}
