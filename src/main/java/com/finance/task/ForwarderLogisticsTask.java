 ...
 *
 */
public class ForwarderLogisticsTask extends QuartzJobBean {

	private final Logger LOGGER = LoggerFactory.getLogger(ForwarderLogisticsTask.class);

	@Override
	protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
		LOGGER.info("ForwarderLogisticsTask is begin ...");
		ForwarderLogisticsService forwarderLogisticsService = SpringUtil.getBean("ForwarderLogisticsService");
		long result2 = forwarderLogisticsService.getCountNOLocation();
		recursion(result2, forwarderLogisticsService);
	}
	
	public void recursion(long l, ForwarderLogisticsService serviceLogisticsService){
		if(l > 0){
			LOGGER.info("开始处理。。。");
			...
			LOGGER.info("ForwarderLogisticsService is over ...");
		}
	}
}
