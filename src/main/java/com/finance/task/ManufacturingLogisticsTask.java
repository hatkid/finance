 ...
 *
 */
public class ManufacturingLogisticsTask extends QuartzJobBean {

	private final Logger LOGGER = LoggerFactory.getLogger(ManufacturingLogisticsTask.class);

	@Override
	protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
		LOGGER.info("ManufacturingLogisticsTask is begin ...");
		
		ManufacturingLogisticsService manufacturingLogisticsService = SpringUtil.getBean("ManufacturingLogisticsService");
		long result4 = manufacturingLogisticsService.getCountNOLocation();
		recursion(result4, manufacturingLogisticsService);
	}
	
	public void recursion(long l, ManufacturingLogisticsService serviceLogisticsService){
		if(l > 0){
			LOGGER.info("开始处理。。。");
			...
			LOGGER.info("ManufacturingLogisticsService is over ...");
		}
	}
