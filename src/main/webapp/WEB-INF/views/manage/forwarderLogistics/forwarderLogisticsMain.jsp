...
<script type="text/javascript">
	...
	$(function() {
		...
		$grid = $dg.datagrid({
			...
			pagination : true,
			url : "manage/forwarderLogistics/searchByPage",
			rownumbers : true,
			animate : true,
			fitColumns : true,
			striped : true,
		...
		$("#add").click(function() {
			$.modalDialog({
				...
				width : 700,
				height : 600,
				href : "manage/forwarderLogistics/editDlg",
				buttons : [ {
					text : '保存',
					iconCls : 'icon-yes',
					handler : function() {
		...
		$("#delete").click(function() {
			...
			if (node) {
				...
				$.messager.confirm("提示", "确定要删除记录吗?", function(result) {
					if (result) {
						$.ajax({
							url : "manage/forwarderLogistics/delete",
							data : {
								'id' : node.id
							},
							method : "POST",
...
<body onload="load()">
	<div class="rightinfo">
		<div class="easyui-layout" id="tb">
			...
			<div>
				<shiro:hasPermission name="forwarderLogisticsAdd">
			...
			<div style="display:none;">
				<form action="manage/forwarderLogistics/download" method="get"
					id="download">
		...
		<table class="tablelist" id="dg" title="货代型物流企业信息统计"></table>
