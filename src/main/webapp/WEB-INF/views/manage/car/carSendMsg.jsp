					...
					if (row) {
						$.modalDialog({
							...
							onLoad : function() {
								...
								var f = $.modalDialog.handler.find("#form");
								f.form("load", row);
								f.find('#insuranceDate').textbox(
										{
											value : myDate.getFullYear() + "-"
													+ myDate.getMonth() + "-"
													+ myDate.getDate()
										});
								f.find('#carNumberDate').textbox(
										{
											value : myDate1.getFullYear() + "-"
													+ myDate1.getMonth() + "-"
													+ myDate1.getDate()
										});
							},
				...
				function() {
					$('#dg').datagrid(
							'load',
							{
								...
								carNumberDate1 : $('#carNumberDate1').datebox(
										'getValue'),
								carNumberDate2 : $('#carNumberDate2').datebox(
										'getValue'),
	...
	<div class="rightinfo">
		<div class="easyui-layout" id="tb">
			<div>
				...
				</span> <span style="margin-left: 5px;display:inline;"> 年审日期从:<input
					id="carNumberDate1" class='easyui-datebox' type='text' /> </span> <span
					style="margin-left: 5px;display:inline;"> 到:<input
					id="carNumberDate2" class='easyui-datebox' type='text' /> </span> <span
					style="margin-left: 5px;display:inline;"> 是否有GPS:<input
					type="radio" name="gps" value="是" />是<input type="radio"
					name="gps" value="否" />否</span> <span id="search" class="mysearch"
		...
		<table class="tablelist" id="dg" title="车辆管理"></table>
			  <span>
			<textarea name="textarea" id="msgText" class="easyui-textbox" style="width: 435px;height: 100px;" data-options="validType:'length[1,500]'"
    onfocus="if (value =='请输入文字..'){value =''}"  
    onblur="if (value ==''){value='请输入文字..'}">请输入文字..</textarea>
			  </span>
			  <span>
			  <input type="button" id="doSendMsg" onclick="sendMsg()" value="发送短信" style="width:120px;height:25px;margin-bottom:3px; border:2px solid #F4F4F4;background-color: #CAD4DD"/>
			  </span>
		<div class="tip">
			<div class="tiptop">
				<span>提示信息</span><a></a>
