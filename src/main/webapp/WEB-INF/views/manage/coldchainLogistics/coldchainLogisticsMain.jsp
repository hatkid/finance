...
<script type="text/javascript">
	...
	$(function() {
		...
		$grid = $dg.datagrid({
			...
			pagination : true,
			url : "manage/coldchainLogistics/searchByPage",
			rownumbers : true,
			animate : true,
			fitColumns : true,
			striped : true,
			...
			columns : [ [{
				...
				title : '冷藏运输车辆数（吨位）与温度',
				align : 'left',
				editor : 'text'
			}, {
				field : 'storageAreaUtilization',
				title : '冷库利用率（%）',
				align : 'left',
				editor : 'text'
			}, {
		...
		$("#add").click(function() {
			$.modalDialog({
				...
				width : 700,
				height : 600,
				href : "manage/coldchainLogistics/editDlg",
				buttons : [ {
					text : '保存',
					iconCls : 'icon-yes',
					handler : function() {
		...
		$("#delete").click(function() {
			...
			if (node) {
				...
				$.messager.confirm("提示", "确定要删除记录吗?", function(result) {
					if (result) {
						$.ajax({
							url : "manage/coldchainLogistics/delete",
							data : {
								'id' : node.id
							},
							method : "POST",
...
<body onload="load()">
	<div class="rightinfo">
		<div class="easyui-layout" id="tb">
			...
			<div>
				<shiro:hasPermission name="coldchainLogisticsAdd">
				...
				<br /> <span style="margin-left: 5px;display:inline;"> 企业名称:
					<input id='companyName' data-options="validType:'length[1,100]'"
							...
							type='text' />
				</span> <span style="margin-left: 5px;display:inline;"> 企业法人: <input id='legalPerson' data-options="validType:'length[1,50]'"
			...
			<div style="display:none;">
				<form action="manage/coldchainLogistics/download" method="get"
					id="download">
		...
		<table class="tablelist" id="dg" title="冷链物流企业信息统计"></table>
