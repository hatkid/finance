...
<script type="text/javascript">
	...
	$(function() {
		...
		$grid = $dg.datagrid({
			...
			pagination : true,
			url : "manage/storageLogistics/searchByPage",
			rownumbers : true,
			animate : true,
			fitColumns : true,
			striped : true,
			...
			columns : [ [{
				...
				field : 'stockType',
				title : '存货类型（家电、纺织品、日化用品、食品等）',
				align : 'left',
				editor : 'text'
			}, {
				field : 'totalThroughput',
				title : '年度总吞吐量（吨或m³）',
				align : 'left',
				editor : 'text'
			}, {
				field : 'storageAreaAverageUtilization',
				title : '年度仓储面积平均利用率',
				align : 'left',
				editor : 'text'
			}, {
		...
		$("#add").click(function() {
			$.modalDialog({
				...
				width : 700,
				height : 600,
				href : "manage/storageLogistics/editDlg",
				buttons : [ {
					text : '保存',
					iconCls : 'icon-yes',
					handler : function() {
		...
		$("#delete").click(function() {
			...
			if (node) {
				...
				$.messager.confirm("提示", "确定要删除记录吗?", function(result) {
					if (result) {
						$.ajax({
							url : "manage/storageLogistics/delete",
							data : {
								'id' : node.id
							},
							method : "POST",
...
<body onload="load()">
	<div class="rightinfo">
		<div class="easyui-layout" id="tb">
			...
			<div>
				<shiro:hasPermission name="storageLogisticsAdd">
			...
			<div style="display:none;">
				<form action="manage/storageLogistics/download" method="get"
					id="download">
		...
		<table class="tablelist" id="dg" title="仓储型物流企业信息统计"></table>
