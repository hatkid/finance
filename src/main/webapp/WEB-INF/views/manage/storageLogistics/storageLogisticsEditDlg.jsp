...
<script type="text/javascript">
	$(function() {
		$("#form").form({
			url : "manage/storageLogistics/saveOrUpdate",
			onSubmit : function() {
				$.messager.progress({
					title : '提示',
					text : '数据处理中，请稍后....'
		...
		<form id="form" method="post">
			<fieldset>
				...
				<table>
					...
					<tr>
						<th>自拥有仓储或租赁仓储</th>
						<td><input name='isRentStorage' id='isRentStorage' data-options="validType:'length[1,100]'"
							class='easyui-textbox easyui-validatebox' type='text' />
						</td>
						<th>仓储面积（平方米）</th>
					...
					<tr>
						<th>仓储容积（立方米）</th>
						<td><input name='storageCapacity' id='storageCapacity' data-options="validType:'length[1,6]'"
							size='26' class='easyui-numberbox'  type='text' />
						</td>
						<th>仓储类型（普仓或特殊仓）</th>
					...
					<tr>
						<th>存货类型（家电、纺织品、日化用品、食品等）</th>
						<td><input name='stockType' id='stockType' data-options="validType:'length[1,100]'"
							class='easyui-textbox easyui-validatebox' type='text' />
						</td>
						<th>年度总吞吐量（吨或m³）</th>
						<td><input name='totalThroughput' id='totalThroughput' data-options="validType:'length[1,6]'"
							size='26' class='easyui-numberbox'  type='text' />
						</td>
					</tr>
					<tr>
						<th>年度仓储面积平均利用率</th>
						<td><input name='storageAreaAverageUtilization' data-options="validType:'length[1,6]'"
							id='storageAreaAverageUtilization' size='26'
							class='easyui-numberbox'  type='text' />
						</td>
						<th>1-5月份仓储面积利用率</th>
